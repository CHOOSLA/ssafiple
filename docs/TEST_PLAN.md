...
> **갱신 (2026-07-16)**: 장소별 WebSocket 실시간 익명 채팅(feature.md §5.4)이 구현 완료되어 관련 항목(UT-WS-*, IT-04, §3 알려진 갭)을 실제 코드 기준으로 반영함.
...
### 1.3.1 장소별 실시간 채팅 (WebSocket) — `routers/chat.py`, `services/chat_service.py`, `utils/websocket_manager.py`

`TestClient.websocket_connect("/api/chat/ws/{location_id}")` 사용. 수신 메시지 규격은 `ChatWsIncoming` (`content` 1~300자), 저장 모델은 `chat_messages` 테이블.

| ID | 케이스 | 입력 | 기대 결과 |
|---|---|---|---|
| UT-WS-01 | 존재하지 않는 장소 접속 | 없는 `location_id`로 connect | `close code=4404`로 즉시 종료 |
| UT-WS-02 | 접속 시 닉네임 발급 | 유효 `location_id`로 connect | 첫 수신 JSON이 `{"type": "self", "nickname": "익명 ..."}` (`generate_nickname` 포맷) |
| UT-WS-03 | 입장 시스템 메시지 broadcast | 같은 방에 2번째 소켓 접속 | 기존 소켓이 `{"type": "system", "content": "...입장했습니다."}` 수신 |
| UT-WS-04 | 메시지 송신 → 저장 + broadcast | `{"content": "안녕"}` 전송 | 같은 방 전 소켓이 `type=message` 수신, DB `chat_messages`에 1건 저장 |
| UT-WS-05 | 규격 위반 메시지 무시 | `content` 누락/빈 문자열/301자 전송 | ValidationError는 무시(`continue`), 연결 유지·broadcast 없음 |
| UT-WS-06 | 퇴장 처리 | 소켓 disconnect | 남은 소켓이 `퇴장했습니다` 시스템 메시지 수신, 빈 방은 `_rooms`에서 제거 |
| UT-WS-07 | 방 격리 | 서로 다른 `location_id` 두 방에 각각 접속 후 한쪽에 송신 | 다른 방 소켓은 미수신 (장소별 Pub/Sub 격리) |
| UT-WS-08 | 최근 메시지 이력 REST 조회 | `GET /api/chat/rooms/{location_id}/messages` | 200, 최근 50건을 오래된 순으로 반환 (`get_recent_messages` reversed 확인) |
| UT-WS-09 | 이력 조회 404 | 없는 `location_id`로 이력 조회 | 404 "관광지 정보를 찾을 수 없습니다." |
...
7. `실시간 채팅` 탭 클릭 → WebSocket 연결 후 발급된 익명 닉네임 표시, 메시지 입력/전송 시 즉시 렌더링 확인
8. 브라우저 탭 2개로 같은 장소 채팅방 진입 → 한쪽에서 보낸 메시지가 다른 탭에 실시간 수신되는지, 입장/퇴장 시스템 메시지가 뜨는지
9. 탭 재진입 시 `GET /api/chat/rooms/{location_id}/messages`로 이전 대화 이력이 복원되는지
...
| 항목 | 상태 |
|---|---|
| ~~WebSocket 실시간 익명 채팅~~ | ✅ **구현 완료** (2026-07-16) — BE `routers/chat.py` WebSocket 엔드포인트 + `websocket_manager.py` 방별 broadcast + `chat_messages` 영구 저장, FE `PlaceChatPanel.vue`(`PostListView.vue`에서 사용) + `chatStore.js` 연동. §1.3.1 UT-WS-* 케이스 참조 |
| `frontend/src/components/board/PostCard.vue`, `CommentItem.vue` | 라우터/페이지 어디서도 import되지 않는 미사용 컴포넌트 — 테스트 대상 제외, 정리 필요 여부 확인 |
| `frontend/src/components/chat/ChatBotWidget.vue` | 미사용 컴포넌트 (`ChatWidget.vue`가 실제 사용 중) — 정리 필요 여부 확인. ※ `PlaceChatPanel.vue`는 실시간 채팅 구현으로 실사용 전환됨 |
| `backend/app/services/chat_service.py` | ~~빈 파일~~ → 실시간 채팅 서비스 계층으로 사용 중 (닉네임 발급, 메시지 저장/조회). AI 챗봇 로직(`services/chat.py`)과 별개 모듈 |
| `backend/app/services/seed_service.py` | 빈 파일 (실제 시딩 로직은 `scripts/seed.py`) |
...
> 3. AI 챗봇 & 실시간 채팅 — `/api/chat`(OpenAI 연동, 대화 히스토리 유지, 비용 통제로 히스토리 턴수 제한) 구현됨. 장소별 WebSocket 실시간 익명 채팅도 구현됨(`/api/chat/ws/{location_id}`, 익명 닉네임 자동 발급, `chat_messages` 테이블 영구 저장, 최근 이력 REST 조회 `/api/chat/rooms/{location_id}/messages`).
